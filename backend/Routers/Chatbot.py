...
from Routers.Stockideas import StockDatas
...
def generate_stock_analysis(symbol, query, db: Session):
    ...
    if symbol:
        stock_data = StockDatas(symbol,db=db)  # Fetch stock data
...
def Bot(ChatModel: Message, db: Session =  Depends(get_db)):
    ...
    if user_input in greetings:
        ...
        if entities:
            if entities['symbol']:
                result = generate_stock_analysis(entities['symbol'], user_input, db)
                return JSONResponse(result)
            elif entities['company']:
                result = generate_stock_analysis(entities['company'], user_input, db)
                return JSONResponse(result)
            else:
