...
from models import Symbols, StockInfo, StockPerformance, CompanyProfile, FinancialMetrics, TechnicalIndicator, FinancialGrowth, StandardDeviation
...
def DebtFree_Stocks(
    ...
    latest_financialgrowth = db.query(
...
def SearchSymbols(symbol: str, db: Session = Depends(get_db)):

    query = db.query(Symbols.Csymbol,Symbols.Cname).order_by(Symbols.Csymbol.asc())
...
def GraphData(symbol: str,range_type: str, db: Session = Depends(get_db)):
...
def StockDatas(symbol : str,db: Session):
    latest_stockinfo = db.query(
        StockInfo.symbol,
        func.max(StockInfo.id).label("latest_id")
    ).group_by(StockInfo.symbol).subquery()
